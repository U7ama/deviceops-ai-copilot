...
## Decision
We enforce Row-Level Security (RLS) on all tenant-bound tables in PostgreSQL. The application connects using a non-owner database role (\deviceops_app\) and sets \pp.current_tenant_id\ per transaction context.
